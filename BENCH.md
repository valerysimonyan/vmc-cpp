...
## Phase 6.3 -- two precision experiments

Two independent compile-time flags in constants.h, FP64 remaining the default and the oracle:

- **`fp32_forward`**: the three networks' GEMM chains (value and jet) run in float. Everything they feed stays FP64 -- xi accumulation, S_combine, the Slater matrices, dets, Minv, the jet algebra, backprop and O assembly. The cast-up boundaries are listed in one comment in `net_forward.h`. Parameters keep the FP64 canonical array plus a float mirror refreshed by one cast kernel per upload (bytes_up unchanged). The activation stash backprop reads stays FP64, so the FP64-backprop claim is unaffected; a float stash would halve its bandwidth and is left as a future refinement.
- **`fp32_opool`**: O_pool is stored as float. Backprop writes FP64 rows into a staging buffer of `opool_stage_rows` walkers and `o_finalize` casts them down -- the single cast-down point. Every consumer accumulates in FP64: `S_diag` reads float and accumulates double, and the four `Dgemv` call sites become two custom kernels (`O^T x` split into 32 slabs summed in a fixed order; `O v` one block per row with a fixed tree reduction), both bitwise-deterministic. `cublasGemmEx` cannot be used: CUBLAS_COMPUTE_64F requires 64-bit inputs.

### Task B: the validity guard had to be retuned first

The guard drops a sample when |psi_jet - psi_double| > tol * max(1, |psi|). Measured on thermalised walkers (`tests/test_precision.cu`, 8192 samples):

| wavefunction | FP64 p99.9 | fp32 p99.9 | fp32 max | rejected at tol = 1e-6 |
|---|---:|---:|---:|---:|
| trained {64} | 9.4e-16 | 7.8e-7 | 2.5e-6 | 3 of 8192 |
| random {64,64,64} | -- | 2.5e-6 | 4.9e-6 | 153 of 8192 |
| **trained {32,32,32}** | 1.0e-14 | **8.9e-6** | 1.5e-5 | **3960 of 8192 (48%)** |

Float error compounds with depth, and training raises it further (the trained {32,32,32} is 3.5x its untrained value). At the FP64 tolerance the guard would have silently discarded half the samples of the deepest trained network. 10x the worst p99.9 is 8.9e-5, so **psi_mismatch_tol = 1e-4 under fp32_forward** (1e-6 in FP64): 0 of 8192 rejected, and n_invalid stayed 0 in every timing and physics run below. Re-measure on new checkpoints with `VMC_PRECISION_CKPT=<file> ./test_precision`.

### Task E1: timing

Li6 probe configuration, one probe per flag combination, network architecture and SR regime. GPU 0, idle card, 49 iterations averaged at lambda = 100 and 9 at the lambda floor (sd 3-40 ms). ms/iter:

| | {64} lambda=100 | {64} lambda floor | {64,64,64} lambda=100 | {64,64,64} lambda floor | memory {64} / {64,64,64} |
|---|---:|---:|---:|---:|---:|
| FP64 | 1248 | 1697 | 2435 | 3283 | 4.90 / 7.20 GiB |
| **fp32_forward** | **468 (2.67x)** | 911 (1.86x) | **678 (3.59x)** | 1551 (2.12x) | 5.54 / 7.84 GiB |
| fp32_opool | 1284 (0.97x) | 1855 (0.91x) | 2474 (0.98x) | 3537 (0.93x) | **4.09 / 5.50 GiB** |
| both | 495 (2.52x) | 1079 (1.57x) | 720 (3.38x) | 1836 (1.79x) | 4.73 / 6.14 GiB |

Stage detail at lambda = 100 (ms/iter, {64} -> {64,64,64}):

| stage | FP64 | fp32_forward |
|---|---|---|
| sweeps (metro) | 318 -> 609 | 97 (3.3x) -> 115 (5.3x) |
| local_E | 193 -> 365 | 60 (3.2x) -> 72 (5.1x) |
| O assembly | 38 -> 86 | 41 -> 95 (slightly slower: the stash is still FP64 and now written by a cast) |
| SR | 79 -> 180 | 80 -> 173 (untouched by this flag) |

### Task E2: physics

**Same wavefunction, both precisions** -- the measurement that isolates a precision bias from training luck. One checkpoint, one seed, evaluated by `frozen_eval` in each build:

| system | FP64 | fp32_forward | shift | statistical error |
|---|---|---|---:|---:|
| deuteron ({64}) | -2.192576 | -2.192140 | +0.44 keV | 2.4 keV |
| triton ({64}) | -8.364497 | -8.363959 | +0.54 keV | 3.8 keV |
| Li6 ({64,64,64}) | -27.172596 | -27.168817 | +3.8 keV | 47 keV |

The fp32 forward pass shifts the energy up by 1-2e-4 of |E|, always in the variational direction (less bound), and in every case far below the statistical error of the evaluation that produced it. `fp32_opool` changes a frozen evaluation by exactly nothing: with no gradients accumulated the pool is never read (identical digits, both systems).

**Independent training runs**, 5000 iterations from scratch (deuteron, triton). These differ by training luck, not precision, and are reported for the invalid-rate monitor and the end point:

| | last-500 mean E | var | invalid/iter | ms/iter |
|---|---|---|---|---|
| deuteron FP64 | -2.2294 | 0.80 | 0 | 234 |
| deuteron fp32_forward | -2.2294 | 0.82 | 0 | 136 |
| deuteron both | -2.2274 | 1.04 | 0 | 167 |
| triton FP64 | -8.3784 | 12.98 | 0 | 436 |
| triton fp32_forward | -8.3848 | 9.87 | 0 | 237 |
| triton both | -8.3867 | 9.74 | 0 | 309 |

The deuteron reaches the same energy in FP64 and fp32 to four decimals, ~11 keV above the 6.A reference -2.2403705 and still descending at 5000 iterations. **n_invalid stayed 0 in every run of this phase** -- the sharpest corruption signal available, and it never moved.

**Li6, 500 iterations of training from the same checkpoint** ({64,64,64}, E about -27.2 at the start):

| | ms/iter | iters 0-100 | 200-300 | 400-500 | invalid |
|---|---:|---|---|---|---|
| FP64 | 2549 | -27.746 +/- 0.043 | -27.820 +/- 0.042 | -27.890 +/- 0.041 | 0 |
| fp32_forward | 800 (3.19x) | -27.748 +/- 0.044 | -27.796 +/- 0.039 | -27.920 +/- 0.039 | 0 |
| both | 871 (2.93x) | -27.629 +/- 0.055 | -27.841 +/- 0.039 | -27.871 +/- 0.038 | 0 (max 1) |

The windows agree within their errors, with the variance unchanged.

### Correctness

- All four flag combinations: 15/15 through the tolerance ladder (`tests/test_tolerances.h`, every FP64 bound unchanged). CPU build 7/7.
- **FP64 baseline bit-identical to before 6.3**: 50 Li6 iterations with both flags off, 0 of 1250 CSV fields differ from commit 930c75d.
- Determinism within each flag combination: bitwise, run to run. Under fp32_forward, results depend on `jet_chunk` at float-rounding level (cuBLAS SGEMM picks its kernel, and so its reduction order, by matrix size); at a fixed chunk they are reproducible.
- `fp32_opool` storage cost, isolated: every O entry within one float rounding (5.96e-8) of its FP64 value; the SR step at the lambda floor, solved to CG tol 1e-10 on both pools, moves by **5.3e-8** -- 20,000x below the production CG tolerance of 1e-3.
- Backprop stays FP64 end to end: with identical inputs (stash, dets, Minv, S), device O matches the host fill_O to 1e-12 of the summand scale under fp32_forward.

### Verdicts

**fp32_forward: ON for exploratory runs, OFF for publication numbers.** 2.7x at Li6 {64}, 3.6x at {64,64,64}, 3.2x over 500 Li6 training iterations, 1.7-1.8x on the deuteron and triton. The energy shift on a fixed wavefunction is +1-2e-4 of |E| (+0.44 / +0.54 / +3.8 keV), always less bound, always below the evaluation's own statistical error; training trajectories and variances match FP64; no invalid samples anywhere. **It requires psi_mismatch_tol = 1e-4** (set automatically by the flag), and that value must be re-checked on new, deeper or better-trained networks with `VMC_PRECISION_CKPT=... ./test_precision`.

**fp32_opool: OFF.** It halves the pool (4.90 -> 4.09 GiB at {64}, 7.20 -> 5.50 GiB at {64,64,64}), which will matter for heavier nuclei, and it is numerically harmless -- but it costs 3-9% wall time instead of the ~2x SR speedup the bandwidth arithmetic predicted. The custom mixed-precision GEMVs reach ~520 GB/s against cuBLAS DGEMV's ~900 GB/s; in isolation they still beat cuBLAS per call (2.0 vs 2.4 ms, since they read half the bytes), but inside the application they run at 3.0 ms per call against cuBLAS's 2.35. float4 loads and 4x unrolling both made them slower. The in-application slowdown is not explained by the profiler and is left open.

### Decision table (updated)

| candidate | status after 6.3 |
|---|---|
| 6.1 graphs | done (6.1) |
| 6.2 custom batched LU | **CLOSED** by measurement: custom kernels slower than cuBLAS batched at N >= 9, where the code is heading. FP32 forwards raise the determinant share of the iteration; that is accepted as the cost of the heavier-nuclei direction. |
